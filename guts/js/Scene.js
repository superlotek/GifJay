...
function sceneSetter(arrayName, banker1, banker2) {
  ...
  if (setOn) {
    ...
    playlist.bank[bankSelectorS1].gifs.forEach(function(element) {
      bankBuilderS1.push({
        gif: element.name,
        location: element.location,
        // type: element.type
    ...
    playlist.bank[bankSelectorS2].gifs.forEach(function(element) {
      bankBuilderS2.push({
        gif: element.name,
        location: element.location,
        // type: element.type
  ...
  if (blendModeRandomOn) {
    // ADD THIS TO FILTERS PAGE AS A FUNCTION
    let randomBlendMode = blendModesArray[Init.numRan(blendModesArray.length)].name;
    $(s2).css('mix-blend-mode', randomBlendMode);
    window.opener.$('#scene-section ' + s2).css('mix-blend-mode', randomBlendMode);
