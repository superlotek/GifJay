...
var textOn = 0;
glerp = "";
$('.text').css('text-align', 'center').css('color', '#ffffff');


// TEXT EFFECTS
  Mousetrap.bind("alt+[", function() {
    console.log('TEXT EFFECTS ON');

    if (!textOn) {
      textOn = 1;
      console.log($('#text-fx').val());
      $('.text').show();
      $('.text').html(glerp);
      console.log('SHOW ME TEXT: ' + glerp);
    ...
    // $('.scene').append('<div class="text">' + glerp + '</div>');
  ...
  Mousetrap.bind("alt+]", function() {
    ...
    $('body').append('<form id="formtest"><input type="text" autocomplete="off" id="text-fx" /><input id="text-submit" type="submit" /></form>');
    $('#text-fx').css({'background':'none', 'color': 'transparent', 'border':'none'}).focus();
    $('#text-fx:focus').css('outline', 'none');
    ...
    Mousetrap.bind("alt+\\", function() {
    console.log('TEXT: ENTER SUBMIT/CLEAR');
    glerp = $('#text-fx').val();
  ...
  Mousetrap.bind("ctrl+[", function() {
    ...
    $('.text').css('font-size', '200px').css('line-height', '170px');
  });
